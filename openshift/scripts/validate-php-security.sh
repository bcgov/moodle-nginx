...
check_version_drift() {
    ...
    # Check if actual composer.json matches centralized versions
    local actual_zipstream=$(jq -r '.require["maennchen/zipstream-php"]' "$COMPOSER_DIR/composer.json" 2>/dev/null || echo "unknown")
