...
# =============================================================================
# This file serves as the main entry point for the modular utility structure.
# It loads specialized utility modules and maintains backward compatibility.
# 
# Module Structure:
...
# =============================================================================
# The following functions provide backward compatibility for existing scripts
# that may depend on the old monolithic structure.
...
# =============================================================================
#
# The original 3149-line monolithic utility file has been successfully
# refactored into a modular structure for better maintainability:
#
# 📁 utils/
# ├── 📄 openshift.sh   - Core OpenShift operations (~600 lines)
# │   ├── Resource management (scale_resource, delete_pod, wait_for)
# │   ├── Maintenance & route management (patch_all_routes, verify_route_target)
# │   ├── Secret management (create_or_update_secret, restart_deployment)
# │   ├── Logging & error handling (log_critical_event, send_notification)
# │   └── Validation functions (validate_resource_exists, check_pod_ready)
# │
# ├── 📄 redis.sh       - Redis-specific operations (~300 lines)
# │   ├── Service management (create_redis_services, manage_redis_resources)
# │   ├── Proxy configuration (generate_redis_proxy_config_json)
# │   ├── Probe management (apply_redis_probe_fixes)
# │   └── Scaling updates (update_redis_proxy_after_scaling)
# │
# ├── 📄 database.sh    - Galera/MariaDB operations (~400 lines)
# │   ├── Health checks (check_galera_cluster_health, check_galera_pod_ready)
# │   ├── Auto-healing (auto_heal_galera_cluster, wait_for_galera_cluster_sync)
# │   ├── Content management (find_db_characters, replace_db_characters_from_csv)
# │   └── Migration utilities (should_migrate_by_version, moodle_content_cleanup)
# │
# └── 📄 moodle.sh      - Moodle-specific operations (~400 lines)
#     ├── Course management (find_courses_with_tag, update_course_tag, migrate_courses)
#     ├── Cache operations (clear_moodle_cache, rebuild_course_cache, check_cache_status)
#     ├── Content cleanup (fix_mojibake_files, moodle_content_cleanup)
#     └── Maintenance mode (manage_moodle_maintenance)
#
# =============================================================================
# BENEFITS OF MODULAR STRUCTURE:
# =============================================================================
# ✅ Better maintainability - Functions organized by domain
# ✅ Team collaboration - Multiple developers can work on different modules
# ✅ Code reusability - Modules can be sourced independently
...
# ✅ Reduced complexity - No more 3000+ line files to navigate
#
# =============================================================================
# USAGE EXAMPLES:
# =============================================================================
# 
# 1. Source all modules (current behavior):
#    source openshift/scripts/_utils.sh
#
# 2. Source specific modules only:
#    source openshift/scripts/utils/openshift.sh
#    source openshift/scripts/utils/redis.sh
#
# 3. Use core functions:
#    scale_resource "statefulset" "mariadb-galera" 3
#    manage_maintenance_mode "enable" "Upgrading system"
#
# 4. Use Redis functions:
#    create_redis_services
#    update_redis_proxy_after_scaling 6
#
# 5. Use Database functions:
#    check_galera_cluster_health "app.kubernetes.io/name=mariadb-galera"
#    auto_heal_galera_cluster "app.kubernetes.io/name=mariadb-galera"
#
# 6. Use Moodle functions:
#    find_courses_with_tag "archived" "$moodle_pod"
#    clear_moodle_cache "$moodle_pod" "all"
#
# =============================================================================
# MIGRATION NOTES:
# =============================================================================
# All existing function calls will continue to work without modification.
# The modular structure provides the same API while improving organization.
# Functions have been enhanced with better error handling and logging.
#
# =============================================================================

echo "🎉 Modular utility structure loaded successfully!"
echo "📋 Available modules: OpenShift Core, Redis, Database, Moodle"
echo "📚 Function count: ~50+ core functions across 4 specialized modules"
echo "🔧 Backward compatibility: All legacy function calls supported"
