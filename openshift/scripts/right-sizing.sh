...
do
  ...
  then
      ...
      echo "Executing: oc patch dc $Deployment -p={\"spec\":{\"strategy\":{\"rollingParams\":{\"maxSurge\":\"$maxSurge\", \"maxUnavailable\":\"33%\"}}}}"
      oc patch dc $Deployment -p="{\"spec\":{\"strategy\":{\"rollingParams\":{\"maxSurge\":\"$maxSurge\", \"maxUnavailable\":\"33%\"}}}}"
  fi
done
